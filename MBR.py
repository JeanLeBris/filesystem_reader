...
class MBR_entry(partitionsystem_element):
    type_int: int
    ...
    def process_data(self):
        ...
        if self.type == "Extended CHS" or self.type == "Extended LBA":
            self.element = EBR(self.parent_filesystem.input_path, self.partition_type_whitelist, self.start, self.start)
...
class MBR(partitionsystem):
    ...
    def analyse_boot_sector(self):
        ...
        partition_entry = MBR_entry(self, 1)
        self.elements.append(partition_entry)
        # partition_entry.process_data()
        # if not(partition_entry.check_if_exist()):
        #     # self.elements.append(partition_entry)
        #     self.elements.pop(-1)
        partition_entry = MBR_entry(self, 2)
        self.elements.append(partition_entry)
        # partition_entry.process_data()
        # if not(partition_entry.check_if_exist()):
        #     # self.elements.append(partition_entry)
        #     self.elements.pop(-1)
        partition_entry = MBR_entry(self, 3)
        self.elements.append(partition_entry)
        # partition_entry.process_data()
        # if not(partition_entry.check_if_exist()):
        #     # self.elements.append(partition_entry)
        #     self.elements.pop(-1)
        partition_entry = MBR_entry(self, 4)
        self.elements.append(partition_entry)
        # partition_entry.process_data()
        # if not(partition_entry.check_if_exist()):
        #     # self.elements.append(partition_entry)
        ...
        self.elements = self.flatten_elements()
    ...
    if args.action == "analyse":
        ...
        for partition in filesystem.elements:
            if partition.is_readable():
                partition.element.root_dir.display_self_data()
                partition.element.root_dir.display_tree_data(1)
        ...
        for partition in filesystem.elements:
            if partition.is_readable():
                content = partition.element.get_filesystem_entry_content(args.path)
                if content != None:
                    break
        if content != None:
